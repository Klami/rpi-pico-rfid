...
void MFRC522::PCD_Init()
{
	...
	if (_resetPowerDownPin != UNUSED_PIN)
	{
		...
		gpio_set_dir(_resetPowerDownPin, GPIO_IN);

		if (gpio_get(_resetPowerDownPin) == 0)
		{												// The MFRC522 chip is in power down mode.
			gpio_set_dir(_resetPowerDownPin, GPIO_OUT); // Now set the resetPowerDownPin as digital output.
			gpio_put(_resetPowerDownPin, 0);			// Make sure we have a clean 0 state.
			busy_wait_us(2);							// 8.8.1 Reset timing requirements says about 100ns. Let us be generous: 2μsl
			gpio_put(_resetPowerDownPin, 1);			// Exit power down mode. This triggers a hard reset.
...
bool MFRC522::MIFARE_OpenUidBackdoor(bool logErrors)
{
	...
	if (received != 1 || response[0] != 0x0A)
	{
		if (logErrors)
		{
			printf("Got bad response on backdoor 0x40 command: %X (%d valid bits)\r\n", response[0], validBits);
		}
		return false;
	}
	...
	if (received != 1 || response[0] != 0x0A)
	{
		if (logErrors)
		{
			printf("Got bad response on backdoor 0x43 command: %X (%d valid bits)\r\n", response[0], validBits);
		}
		return false;
	}
