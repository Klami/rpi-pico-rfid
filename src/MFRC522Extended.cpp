...
MFRC522::StatusCode MFRC522Extended::PICC_RequestATS(Ats *ats)
{
	...
	// ------------+-----+-----+-----+-----+-----+-----+-----+-----+-----+-----------
	// FSD ( uint8_ts) |  16 |  24 |  32 |  40 |  48 |  64 |  96 | 128 | 256 | RFU > 256
	//
	...
	// ------------+-----+-----+-----+-----+-----+-----+-----+-----+-----+-----------
	// FSC ( uint8_ts) |  16 |  24 |  32 |  40 |  48 |  64 |  96 | 128 | 256 | RFU > 256
	//
	// Default FSCI is 2 (32  uint8_ts)
	if (ats->size > 0x01)
	{
		...
		case 0x08:
			...
			// The reason I ignore it is that MFRC255 FIFO is 64  uint8_ts so this is not a possible value (or atleast it shouldn't)
			// ats->fsc = 256;
			break;
	...
	else
	{
		...
		// Default FSCI
		ats->fsc = 32; // Defaults to FSCI 2 (32  uint8_ts)
...
)
{
	...
	if (result == STATUS_OK)
	{
		...
		// We should receive our PPS  uint8_t and 2 CRC  uint8_ts
...
MFRC522::StatusCode MFRC522Extended::TCL_Transceive(PcbBlock *send, PcbBlock *back)
{
	...
	{
		printf("CRC is not taken care of by MFRC522: 0x%02X", rxModeReg);
...
{
	// ATQA
	printf("Card ATQA: 0x%04X\r\n", tag->atqa);

	// UID
	printf("Card UID:");
	for (uint8_t i = 0; i < tag->uid.size; i++)
	{
		printf("%02X", tag->uid.uidByte[i]);
	}
	printf("\r\n");
...
void MFRC522Extended::PICC_DumpISO14443_4(TagInfo *tag)
{
	...
	{ // The first byte is the ATS length including the length byte
		...
		{
			printf("%02X", tag->ats.data[offset]);
		}
		printf("\r\n");
	}
