...
public:
	...
	typedef struct
	{
		...
		// Raw data from ATS
		uint8_t data[FIFO_SIZE - 2]; // ATS cannot be bigger than FSD - 2 uint8_t s (CRC), according to ISO 14443-4 5.2.2
	} Ats;
	...
	StatusCode TCL_Transceive(TagInfo *tag, uint8_t *sendData, uint8_t sendLen, uint8_t *backData = nullptr, uint8_t *backLen = nullptr);
	StatusCode TCL_TransceiveRBlock(TagInfo *tag, bool ack, uint8_t *backData = nullptr, uint8_t *backLen = nullptr);
